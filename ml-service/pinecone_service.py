...
def upsert_embedding(index, vector_id: str, embedding: list, metadata: dict = None):
    ...
    try:
        ...
        print(f"Successfully upserted vector: {vector_id}", file=sys.stderr)
    except Exception as e:
        print(f"An error occurred during upsert: {e}", file=sys.stderr)
...
def delete_user_vectors(index, user_id: str):
    ...
    try:
        ...
        print(f"Deleted existing vectors for userId: {user_id}", file=sys.stderr)
    except Exception as e:
        # Ignore missing namespace errors on fresh indexes; log others
        msg = str(e)
        if "Namespace not found" in msg:
            print("Namespace not found on delete; continuing (fresh index).", file=sys.stderr)
        else:
            print(f"An error occurred during delete: {e}", file=sys.stderr)
